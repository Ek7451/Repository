...
- The configurator becomes the only interactive route. `/`, the legacy dashboard URL, and direct configurator URLs all funnel into the same bootstrap in [app.js](/c:/Users/Elliott%20Klinger/Desktop/Repository/app.js).
...
- Phase 1: remove the dashboard boot path from [app.js](/c:/Users/Elliott%20Klinger/Desktop/Repository/app.js), stop routing missing-session users to the dashboard, and make the legacy dashboard HTML a redirect-only compatibility shell. Delete `DashboardPage` usage after the cutover is complete.
...
## Test Plan
- `/`, `/pages/dashboard/dashboard.html`, and `/pages/configurator/index.html` all end in the configurator flow, and `?project=` still opens that project.
...
- The old dashboard route remains only as a redirect for compatibility; `pages/dashboard/dashboard.js` and `ui/project-dashboard.js` should be removed once unreferenced.
