...
function buildLegacyMetricsFromLayout({
    ...
    return {
        ...
        wheelchairSpacesAreaSqFt: Math.max(0, Number(accessibilitySummary?.wheelchairSpacesAreaSqFt) || 0),
        companionSpacesAreaSqFt: Math.max(0, Number(accessibilitySummary?.companionSpacesAreaSqFt) || 0),
        totalAccessibilityAreaSqFt: Math.max(0, Number(accessibilitySummary?.totalAccessibilityAreaSqFt) || 0),
        ...
        // This legacy display field is estimate-only; the authoritative layout path does not recompute it.
